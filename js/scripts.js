(function ($) {
	...
	$(document).ready(function () {
		...
		/*==========  Blog  ==========*/
		let $blogContainer = $('#blog-masonry').imagesLoaded(function() {
		...
		let url = 1;
		let count = $('.sections-wrapper section').length;
		if (location.hash) {
			setTimeout(function() {
				window.scrollTo(0, 0);
			}, 1);
	...
	$('.contact-form').on('submit', function() {
		...
		if (validateEmail(contactForm.find('.contact-email').val()) && contactForm.find('.contact-email').val().length !== 0 && contactForm.find('.contact-name').val().length !== 0 && contactForm.find('.contact-message').val().length !== 0) {
			let action = contactForm.attr('action');
			$.ajax({
				type: "POST",
				url : action,
				data: {
